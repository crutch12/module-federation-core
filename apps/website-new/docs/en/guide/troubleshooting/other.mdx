...
## Problem: esm `remoteEntry.js` usage in var environment (without `@module-federation/runtime`)

### Problem

There are some cases when your `host` application is written with [ModuleFederationPlugin](https://webpack.js.org/plugins/module-federation-plugin/) and you are not able (or don't want) to rewrite it, but you want to use esm (e.g. `vite`) in your `remote` application.

In this case you are not able to load `remote` esm application without `@module-federation/runtime`.
